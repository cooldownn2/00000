...
local function applyValue(settings, value, path)
...
function ConfigBootstrap.applyUserConfig(userConfig, settings)
    if type(userConfig) ~= "table" or type(settings) ~= "table" then return end
    for _, entry in ipairs(CONFIG_MAP) do
        applyValue(settings, resolve(userConfig, entry[1]), entry[2])
    end
end

function ConfigBootstrap.validateSettings(Settings)
